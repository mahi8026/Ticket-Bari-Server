...
try {
  const serviceKey = process.env.FB_SERVICE_KEY;
  if (serviceKey) {
  ...
  try {
    ...
    if (booking.status === "paid") {
      return res.status(400).send({
        message: "Cannot delete a paid booking. Contact support for refunds.",
      });
    }
...
async function run() {
  try {
    ...
    console.log(" Connected to MongoDB Atlas"); //await client.db("admin").command({ ping: 1 }); // console.log( //" Pinged your deployment. Successfully connected to MongoDB!" //);
  } catch (e) {
